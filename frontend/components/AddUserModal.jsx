...
const UserCard = (props) => {
  ...
  return (
    <Space align="start" size="large">
      ...
      <Descriptions
        title={<Link href={`/profiles/${address}`}>{name}</Link>}
        column={2}
        size="middle"
        layout="vertical"
        extra={
      ...
      >
        <Descriptions.Item label="所属" span={2}>
          {department}
        </Descriptions.Item>
...
const AddUserModal = ({ visible, onWalletOpen, onMemberAdded, onCancel }) => {
  ...
  const onClickAdd = useCallback(
    (addr) => {
      ...
      (async () => {
        try {
          const tx = await contract.addMembers(DAO_ID, [addr]);
          await tx.wait();
  ...
  const accountSkillsQuery = useAccountsSkills(
    name ?? "",
    useMemo(() => [...selectedSkills], [selectedSkills])
  );
  const accountsQuery = useAccounts();

  const loading = accountSkillsQuery.loading || accountsQuery.loading;

  const accounts = useMemo(() => {
    return (accountSkillsQuery?.data?.accounts ?? []).filter((addr) => {
      const a = (accountsQuery?.data?.accounts ?? []).find(
        (x) => x.address === addr
      );
      if (!a) {
        return false;
      ...
      return a.daos.find((dao) => dao.id === DAO_ID) === undefined;
    });
  }, [accountsQuery.data, accountSkillsQuery.data]);
              ...
              {Skills.map((s) => (
                <Tag
                  ...
                  style={{
                    marginTop: "4px",
                    cursor: "pointer",
                    opacity: selectedSkills.has(s.name) ? "1.0" : "0.3",
                  }}
                  ...
                  return (
                    <>
                      ...
                      <UserCard
                        key={addr}
                        address={addr}
                        isAdding={isAddingMap[addr]}
                        onClickAdd={() => onClickAdd(addr)}
                      />
                    </>
                  );
                })}
